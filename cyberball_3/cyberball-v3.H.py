...
subjDlg.addField('Nombre del jugador:')
...
useFullScreen = True
win = visual.Window([800,600], monitor='testMonitor', units="deg", fullscr=True, allowGUI=False, color="#D3D3D3")

################
# Set up stimuli #
################ 

title=visual.TextStim(win,text="Bienvenido a la sala 'Lanzamiento de Pelota', una tarea interactiva de visualización mental!", height=1.5, pos=(0,6),color="#000000", wrapWidth=30)
instrText = visual.TextStim(win, text="",height=1, color="#000000", wrapWidth=30)
instrKey = visual.TextStim(win, text="", height=.8, color="#000000", pos=(0,-7))
...
def show_instructions():
    ...
    Presiona el botón del índice izquierdo para COMENZAR''')
    instrKey.draw()
    win.flip()
    ...
    title.setText('Bienvenido a la sala de juego LP')
    instr_p1.setText("JUGADOR 1: Esperando jugador")
    instr_p2.setText("JUGADOR 2: Bienvenido %s" % player_name)
    instr_p3.setText("JUGADOR 3: Esperando jugador")
    ...
    for tick in range(500):
        if tick == p1_ticker_end:
            instr_p1.setText("Jugador 1: Bienvenido %s" % player1_name)
            p1_tick.setAutoDraw(False)
        elif tick == p3_ticker_end:
            instr_p3.setText("Jugador 3: Bienvenido %s" % player3_name)
...
def select_throw():
    ...
    if holder==2:
        ...
        choice=[]
        while len(choice)==0 or choice [0] not in ('b','c','q'):
            core.wait(0.01)
            if trialCnt > maxTrials or trialClock.getTime() > maxTime:
                return
            choice = event.waitKeys(keyList=['b','c','q'])
        if choice[0]=='b':
            throwTo=1
